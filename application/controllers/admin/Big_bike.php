...
class Big_bike extends CI_Controller
{
    ...
    function index()
    {
        $x['data'] = $this->m_big_bike->get_all_big_bike();
        $this->load->view('admin/v_big_bike', $x);
    }
    function add_big_bike()
    {
        $this->load->view('admin/v_add_big_bike');
    }
    function get_edit()
    {
        $kode = $this->uri->segment(4);
        $x['data'] = $this->m_big_bike->get_big_bike_by_kode($kode);
        $this->load->view('admin/v_edit_big_bike', $x);
    }
    function simpan_big_bike()
    {
        $config['upload_path'] = './assets/images/big_bike/'; //path folder
        ...
        $this->m_big_bike->simpan_big_bike($judul, $isi, $user_nama, $port_image, $port_image1, $port_image2, $port_image3, $port_image4);
        echo $this->session->set_flashdata('msg', 'success');
        redirect('admin/big_bike');
    }
    ...
    function _create($file_name)
    {
        ...
        $config['source_image'] = './assets/images/big_bike/' . $file_name;
        $config['create_thumb'] = FALSE;
        $config['maintain_ratio'] = FALSE;
        $config['quality'] = '60%';
        $config['new_image'] = './assets/images/big_bike/' . $file_name;
        $this->load->library('image_lib', $config);
        $this->image_lib->resize();
    }

    // fungsi update data beserta banyak gambar dan replace gambar lama
    function update_big_bike()
    {
        ...
        $gambar = $this->db->get_where('tbl_big_bike', array('port_id' => $kode));
        foreach ($gambar->result_array() as $g) {
            ...
            if (!empty($_FILES['port_image']['name'] != null)) {
                ...
                $path = './assets/images/big_bike/' . $g['port_image'];
                unlink($path);
            }
            ...
            if (!empty($_FILES['port_image1']['name'])) {
                ...
                $path = './assets/images/big_bike/' . $g['port_image1'];
                unlink($path);
            }
            //gambar3
            if (!empty($_FILES['port_image2']['name'])) {
                ...
                $path = './assets/images/big_bike/' . $g['port_image2'];
                unlink($path);
            }
            ...
            if (!empty($_FILES['port_image3']['name'])) {
                ...
                $path = './assets/images/big_bike/' . $g['port_image3'];
                unlink($path);
            }
            ...
            if (!empty($_FILES['port_image4']['name'])) {
                ...
                $path = './assets/images/big_bike/' . $g['port_image4'];
                unlink($path);
            }
            ...
            $this->m_big_bike->update_big_bike($port_id, $judul, $isi, $user_nama, $port_image, $port_image1, $port_image2, $port_image3, $port_image4);
    ...
    function hapus_big_bike()
    {
        ...
        foreach ($gambar->result_array() as $g) {
            ...
            $this->m_big_bike->hapus_big_bike($kode);
